...
class ArticleTest(TestCase):
        def test_article_created_success(self):
            ArticleModel.objects.create(title="test article", category="test category", author="test author", content="test content", createdAt=datetime.now(tz=timezone.utc))
            article = ArticleModel.objects.get(title="test article")
            self.assertEqual(article.category, "test category")
            self.assertNotEqual(article.category, "test category2")
